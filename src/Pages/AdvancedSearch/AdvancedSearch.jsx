    ...
    <>
      ...
      <div className="min-h-screen bg-app-bg p-4 theme-page-pad md:p-8" dir="rtl">
        <div className="container mx-auto">
          <h1 className="theme-heading text-3xl theme-title-xl font-black mb-8 text-center">البحث المخصص عن الجامعات والكليات</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              <div className="lg:col-span-2">
                ...
                <input type="text" name="searchTerm" value={filters.searchTerm} onChange={handleChange}
                  placeholder="ابحث باسم الجامعة..." className="theme-input w-full p-4 rounded-2xl outline-none focus:ring-2 focus:ring-brand-500 transition-all text-right" />
              </div>
              ...
              <div className="lg:col-span-2">
                ...
                <input type="text" name="collegeName" value={filters.collegeName} onChange={handleChange}
                  placeholder="مثال: هندسة، طب، علوم..." className="theme-input w-full p-4 rounded-2xl outline-none focus:ring-2 focus:ring-brand-500 transition-all text-right" />
              </div>

              <div>
                <label className="block text-sm font-bold theme-subtle mb-2 mr-2">نوع الجامعة</label>
                <select name="type" value={filters.type} onChange={handleChange} className="theme-input w-full p-4 rounded-2xl outline-none cursor-pointer">
              ...
              <div>
                ...
                <select name="studyType" value={filters.studyType} onChange={handleChange} className="theme-input w-full p-4 rounded-2xl outline-none cursor-pointer">
              ...
              <div>
                ...
                <select name="governorate" value={filters.governorate} onChange={handleChange} className="theme-input w-full p-4 rounded-2xl outline-none cursor-pointer">
              ...
              <div>
                ...
                <input type="number" name="minFees" value={filters.minFees} onChange={handleChange} placeholder="0" className="theme-input w-full p-4 rounded-2xl outline-none font-sans" />
              </div>
              <div>
                <label className="block text-sm font-bold theme-subtle mb-2 mr-2">المصاريف (إلى)</label>
                <input type="number" name="maxFees" value={filters.maxFees} onChange={handleChange} placeholder="100000" className="theme-input w-full p-4 rounded-2xl outline-none font-sans" />
              </div>

              <div>
                <label className="block text-sm font-bold theme-subtle mb-2 mr-2">التنسيق % (من)</label>
                <input type="number" name="minCoordination" value={filters.minCoordination} onChange={handleChange} placeholder="50" className="theme-input w-full p-4 rounded-2xl outline-none font-sans" />
              </div>
              <div>
                <label className="block text-sm font-bold theme-subtle mb-2 mr-2">التنسيق % (إلى)</label>
                <input type="number" name="maxCoordination" value={filters.maxCoordination} onChange={handleChange} placeholder="100" className="theme-input w-full p-4 rounded-2xl outline-none font-sans" />
              </div>

            </div>

            <button type="submit" className="w-full mt-10 bg-brand-600 text-white py-5 rounded-2xl font-black text-xl hover:bg-brand-700 transition-all shadow-xl shadow-brand-100 flex items-center justify-center gap-3 cursor-pointer">
              ...
              {results.length > 0 ? (
                results.map((uni) => (
                  ...
                  className="theme-card theme-elevated rounded-[2.5rem] p-8 shadow-sm hover:shadow-2xl transition-all group">
                    ...
                    <h3 className="theme-heading text-xl font-black mb-2 leading-tight">{uni.nameAr}</h3>
                    <p className="theme-subtle text-sm mb-6 line-clamp-2 italic">{uni.description || "لا يوجد وصف متاح."}</p>
